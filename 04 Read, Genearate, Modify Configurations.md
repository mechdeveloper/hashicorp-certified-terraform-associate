...
- Terraform has capability to output the attribute of a resource with output values
...
- Output attributes can be used for user reference and can also act as input to other resources being created via terraform.
...
- elastic ip - add it to security group rule
  - block style `"${}"` approach to comibne variable with a string
  ```
...
- Create a terraform file `variables.tf` which will act as central source of variable
...
### Multiple ways to assign values in terraform variables
...
- Some of these are 
  - Environment Variables 
    ...
    - set environment variables using `bash`
      ...
      - in Linux you donot need to restart the terminal session and you can check your environment variable in current sesssion 
        ...
        echo $TF_VAR_instancetype
        ```
  - Command Line flags `-var="variablename=variablevalue"`
    ```bash
  ...
  - From `terraform.tfvars` file
    ...
    - if you change the name of the file, for e.g. `custom.tfvars`, terraform by default will not see this file, however you can explicitly tell terraform to look into `custom.tfvars` file while running terraform cli command for eg `terraform plan -var-file="custom.tfvars"`
  - Variable Defaults, if no explicit value is provided then the default value is used
    ```
    variables "instancetype" {
      default = "t2.micro"
    }
    ```
- If you have configured variables and not provided default value for your variable, neither you have passed any value via a `.tfvars` file or command line flag `-var` or via an environment varialbe, then terraform will prompt for you to provide the variable value when running the cli command
  ```
  $ terraform plan
  var.instancetype
    Enter a value: 
...
- Data types 
  ...
  | `map` | a group of values identified by nameed labels, like `{name = "Mabel, age=52}`| 

- It is best practice to explicitly specify type of value that is expected for your variable
- Fetching values from `list` and `maps` variables in terraform
...
- Fetching value from `list` type, `var.variablename[positionnumber]` -
...
- Example of `count` parameter
  ...
  ```
- `count.index` - In resource blocks where count is set, an additional `count` object is available n expressions, so you can modify the configuration of each instance. This `count` object has one attribute: `count.index`, the distinct index number (starting with 0) corresponding to this instance.
...
- use local values only in moderation, in situtations where a single value or result is used in many places and that value is likely to be changd in future 
...
Terraform language does not support user-defined functions, and so only the functions built in to the langugage are available for use.
...
- Reads from a specific datasource `aws_emi` and exports results under `app_emi`

## Debugging in Terraform

- Terraform has detailed logs which can be enabled by setting the `TF_LOG` environment variable to one of the log levels `TRACE`, `DEBUG`, `INFO`, `WARN`, `ERROR` and terraform command will now output the detialed logs in the console.
...
Important points
- `terraform` taint command will not modify infrastructure, but does modify the state file in order to mark a resource as tained.
